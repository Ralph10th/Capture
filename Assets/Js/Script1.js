// Header - Navigation Bar Effects on Scroll
...
forms.addEventListener('submit', e => {
    ...
    const emailpattern = /^[a-zA-Z0-9][-\._a-zA-Z0-9]+@gmail.com$/ ; 

    const message = forms.message.value;
    const messagepattern = message.length;

    if (namepattern.test(name)) {
        if  (emailpattern.test(email)) {
            if (messagepattern >= 30) {
                if (confirm("Are you sure you want to send your message?") == true) {
                ...
                alert("Message must contain atleast 30 letters.")
            }
        } else {
            alert("Type your email correctly!")
        }
   } else {
    alert("Type your full name correctly!")
   }
});
